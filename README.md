...
При первом запуске уведомитель создаёт ярлык в папке автозапуска вашего профиля, чтобы при входе в ваш сеанс Windows уведомитель запускался сам. Пример папки для автозапуска: "C:\Users\kornienko\главное меню\Программы\Startup\". При выполнении пункта контекстного меню уведомителя Quit (Выход), ярлык в папке автозапуска удаляется, программа больше запускаться не будет пока вы её самостоятельно не запустите снова.
### Как скачать
...
0.1.1.0. 07.03.2025. Предусмотрен случай разрыва связи во время получения ответа с IMAP-сервера после установления связи.  
0.2.0.0. 19.11.2025. Изменены иконки прихода почты на более видимые.  
0.3.0.0. 01.12.2025. В ini файл с настройками добавлен параметр showErrors. Ошибки соединения выводится не будут, если showErrors равно 0 или No(в любом регистре), в противном случае уведомления об ошибках соединения выводится будут. Добавлен механизм автозапуска.  
